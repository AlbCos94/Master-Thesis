...
IMU2_COMP = Component('IMU', 3, 12, components=[QUATERNION_COMP, LIN_ACC_COMP])
# Euler Angles
EULER_COMP = Component('e', 3, 3, fields=[])
#changed 10 to 15
#IMU3_COMP = Component('IMU', 3, 10, components=[QUATERNION_COMP, LIN_ACC_COMP, EULER_COMP])
IMU3_COMP = Component('IMU', 3, 15, components=[QUATERNION_COMP, LIN_ACC_COMP, EULER_COMP])
...
#21 + 9 + 9+27
EXO2_EULER = Component('exo2euler', 1, 66, components=[ZMD2_COMP, IMU3_COMP])
...
def parseArgs():
    ...
    parser.add_argument('-v', '--exoversion', help='Select the exo version '
                        + 'with wich the data was created. [exo1|exo2|exo2euler]',
...
def trainModel(trainWindows):
    ...
    truehmms = Parallel(n_jobs=-1)(delayed(trainHMM)(hmm, trainWindows) for hmm in hmms) 
